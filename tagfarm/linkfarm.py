...
class LinkFarmManager:
    ...
    def _get_media_path(self, scene: Dict[str, Any]) -> Path:
        """Get the media path from the scene's first file."""
        files = scene.get("files", [])
        if files:
            source_path = Path(files[0]["path"])
            # Apply path mapping if necessary
            for src_prefix, dest_prefix in getattr(
                self, 'path_map', {}
            ).items():
                if str(source_path).startswith(src_prefix):
                    mapped_path = str(source_path).replace(
    ...
    def create_tag_links(self, tag_name: str, scenes: List[Dict[str, Any]]) -> None:
        ...
        for scene in scenes:
            ...
            source_path = self._get_media_path(scene)
